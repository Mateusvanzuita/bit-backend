// prisma/seedAdmin.js
// Cria o primeiro SUPER_ADMIN e o SosPrompt inicial do SOS Bitzy
// Uso: node prisma/seedAdmin.js
//
// Variáveis de ambiente necessárias no .env:
//   ADMIN_EMAIL=admin@bitzy.app
//   ADMIN_PASSWORD=suaSenhaSegura123
//   ADMIN_NOME=Admin Bitzy
...
/* ========== PROMPT DO SOS BITZY ==========
 * Para atualizar o prompt, edite apenas a constante abaixo.
 */
const SYSTEM_PROMPT = `
...
async function seedSosPrompt(adminId) {
  // Desativa qualquer prompt ativo anterior
  const desativados = await prisma.sosPrompt.updateMany({
    where: { ativo: true },
    data: { ativo: false },
  });

  if (desativados.count > 0) {
    console.log(`⚠️  ${desativados.count} prompt(s) anterior(es) desativado(s).`);
  }

  const prompt = await prisma.sosPrompt.create({
    data: {
      conteudo: SYSTEM_PROMPT,
      ativo: true,
      adminId,
    },
  });

  console.log(`✅ SosPrompt criado e ativado (id: ${prompt.id})`);
}

async function main() {
  const email    = process.env.ADMIN_EMAIL;
  const password = process.env.ADMIN_PASSWORD;
  const nome     = process.env.ADMIN_NOME || 'Admin Bitzy';
  ...
  if (admin) {
    console.log(`⚠️  Admin com email ${email} já existe. Pulando criação.`);
  } else {
    const senha = await bcrypt.hash(password, 12);
    admin = await prisma.admin.create({
      data: { nome, email, senha, role: 'SUPER_ADMIN' },
    });
    console.log(`✅ SUPER_ADMIN criado: ${admin.email} (id: ${admin.id})`);
  }

  // ── SosPrompt ────────────────────────────────────────────
  await seedSosPrompt(admin.id);
}
